  ...
  return (
	...
	<div className="flex justify-between p-4">
		<div className="flex space-x-4">
			<div>
				<img src={story.profileImage} alt="" className="object-cover w-12 h-12 rounded-full dark:bg-gray-500" />
			</div>
			<div>
				<h4 className="font-bold text-gray-900 dark:text-white">{story.name}</h4>
				<span className="text-xs text-gray-900 dark:text-white">{story.date}</span>
			</div>
		</div>
		<div className="flex items-center space-x-2 text-[#2563EB]">
			...
			<span className="text-xl font-bold text-[#2563EB]">{story.rating}</span>
		</div>
	</div>
	<div className="p-4 space-y-2 text-sm text-gray-900 dark:text-white">
        <div><h1 className='font-extrabold text-xl self-center'>Couple Bio : {story.selfBio}💝{story.partnerBio}</h1></div>
		<p>{story.story}</p>
	</div>
</div>
    </div>
